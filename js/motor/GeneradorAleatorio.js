...
export class GeneradorAleatorio {
    ...
    static generarGastoAleatorio(perfilEnum, categoria, claseGasto, localizacion = null, excludeNames = []) {
        let opciones = CatalogoGastos.getGastosPorCategoria(perfilEnum, categoria, localizacion);
    ...
    static generarOleadaSemanal(config, semana, recurrentesGenerados = []) {
        ...
        for (let i = 0; i < numRecurrentes; i++) {
            const g = this.generarGastoAleatorio(config.perfil, "Recurrente", GastoRecurrente, null, recurrentesGenerados);
            if (g) {
                gastos.push(g);
